...
describe("Testing e-commerce", () => {
  ...
  describe("Test de products", () => {
    ...
    it("El endpoint GET /api/products/:pid debe devolver un solo producto", async () => {
      const { _body } = await requester
        .get("/api/products/63d3df5453af636c753a1034")
        .set("Cookie", [`${cookie.name}=${cookie.value}`]);
  ...
  describe("Test de carts", () => {
    ...
    let mockCart
    const data = [{product: "63d3df5453af636c753a102d", quantity: 1}, {product: "63d3df5453af636c753a1034", quantity: 2}]
    ...
    it("El endpoint POST /api/carts/:cid/purchase debe generar un ticket de compra", async () => {
      await requester.post(`/api/carts/${mockCart._id}/products/${pid}`).set("Cookie", [`${cookie.name}=${cookie.value}`]);

      const { _body } = await requester
      .post(`/api/carts/${mockCart._id}/purchase`)
      .set("Cookie", [`${cookie.name}=${cookie.value}`]);
      expect(_body.payload._id).to.be.ok
      expect(_body.payload.purchaser).to.be.eql("test@gmail.com")
    })
  });
});
